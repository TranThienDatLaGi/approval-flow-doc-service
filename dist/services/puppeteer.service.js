 ...
 */
async function renderHtmlToPdf(htmlContent, fieldValues = {}) {
 ...
 */
function injectFieldValues(html, fieldValues) {
    let result = html;
    for (const [key, value] of Object.entries(fieldValues)) {
        // Thay thế span field-tag dạng: <span data-field-key="employee_name" ...>...</span>
        const fieldTagRegex = new RegExp(`<span[^>]*data-field-key="${key}"[^>]*>.*?</span>`, 'gi');
        result = result.replace(fieldTagRegex, `<span style="color:inherit;font-weight:inherit;">${value}</span>`);
 ...
 */
function wrapInHtmlDocument(htmlFragment) {
    return `
<!DOCTYPE html>
<html lang="vi">
<head>
